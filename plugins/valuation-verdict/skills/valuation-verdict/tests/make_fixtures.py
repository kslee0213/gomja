...
def dart_items(year: int, scale: float, half: bool = False) -> dict:
    ...
    bs = [
        ...
        row("BS", "ifrs-full_PropertyPlantAndEquipment", "유형자산", ppe, ord_=8),
    ...
    cf = [
        ...
        row("CF", "ifrs-full_PurchaseOfPropertyPlantAndEquipmentClassifiedAsInvestingActivities", "유형자산의취득", -capex * f, ord_=4),
...
def make_sec(cache: Path) -> None:
    ...
    def add(tag, start, end, val, form, filed, fy, fp, frame=None, unit="USD"):
    ...
    for fy, g in growth.items():
        ...
        add("EarningsPerShareDiluted", y0, y1, ni / shares, "10-K", filed_k, fy, "FY", frame=f"CY{fy}", unit="USD/shares")
        ...
        for q, (qs, qe) in enumerate((("01-01", "03-31"), ("04-01", "06-30"), ("07-01", "09-30")), start=1):
            ...
            add("RevenueFromContractWithCustomerExcludingAssessedTax", f"{fy}-{qs}", f"{fy}-{qe}", rev * 0.24, "10-Q", filed_q, fy, f"Q{q}", frame=f"CY{fy}Q{q}")
            add("NetIncomeLoss", f"{fy}-{qs}", f"{fy}-{qe}", ni * 0.24, "10-Q", filed_q, fy, f"Q{q}", frame=f"CY{fy}Q{q}")
            add("OperatingIncomeLoss", f"{fy}-{qs}", f"{fy}-{qe}", op * 0.24, "10-Q", filed_q, fy, f"Q{q}", frame=f"CY{fy}Q{q}")
            add("Assets", None, f"{fy}-{qe}", rev * 1.45, "10-Q", filed_q, fy, f"Q{q}", frame=f"CY{fy}Q{q}I")
            add("StockholdersEquity", None, f"{fy}-{qe}", rev * 0.68, "10-Q", filed_q, fy, f"Q{q}", frame=f"CY{fy}Q{q}I")
