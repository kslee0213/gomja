...
| `KRX_AUTH_KEY` | (사용자 제공, 선택) | "투자분석" 시트의 주가 연동 지표(PER·PBR·PSR·시가총액)에만 필요하다. 없으면 그 부분만 건너뛰고 나머지는 정상 진행한다. `scripts/fetch_stock_price.py --auth-key <값>` 형태로 전달하며, DART_API_KEY와 마찬가지로 로그·산출물에 노출하지 않는다. KRX Open API는 `data-dbg.krx.co.kr` 도메인을 호출하므로, 이 도메인이 Cowork 네트워크 허용 목록에 없다면 먼저 사용자에게 설정(코드 실행 → 네트워크 송신 허용 → 추가 허용 도메인)을 요청한다. |
...
2. **KRX 종가** (KRX_AUTH_KEY가 있을 때만): 연간 시트에 쓸 각 사업연도 말일 기준으로, 연도별로 한 번씩 실행한다.
   ...
   - **처음 한 번은 반드시 이 명령에 `--raw`를 붙여 실제 응답 구조를 확인**하고, 스크립트 상단 `FIELD_CANDIDATES`의 필드명과 실제 응답이 다르면 그에 맞게 스크립트를 수정한 뒤 재실행한다(KRX Open API 필드명은 문서화가 부실해 사전 검증이 어려웠던 부분).
...
- **E. 주가 연동 지표**: 4-1단계에서 KRX 가격 캐시를 가져온 연도만 종가·시가총액·PER·PBR·PSR을 채운다. 캐시가 없는 연도는 "가격 데이터 없음"으로 표시하고 계산을 건너뛴다.
