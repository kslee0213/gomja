...
| `STOCK_PRICE_SERVICE_KEY` | (사용자 제공, 선택) | "투자분석" 시트의 주가 연동 지표(PER·PBR·PSR·시가총액)에만 필요하다. 없으면 그 부분만 건너뛰고 나머지는 정상 진행한다. 공공데이터포털("금융위원회_주식시세정보") 발급 키를 `scripts/fetch_stock_price.py --service-key <값>` 형태로 전달한다. DART_API_KEY와 마찬가지로 로그·산출물에 노출하지 않는다. `apis.data.go.kr` 도메인을 호출하므로, 허용 목록에 없다면 먼저 사용자에게 설정(코드 실행 → 네트워크 송신 허용 → 추가 허용 도메인)을 요청한다. (참고: KRX Open API는 상품별 개별 승인이 필요해 막히는 사례가 있어 이 스킬에서는 채택하지 않았다.) |
...
2. **주가(종가·시가총액·상장주식수)** (STOCK_PRICE_SERVICE_KEY가 있을 때만): 연간 시트에 쓸 각 사업연도 말일 기준으로, 연도별로 한 번씩 실행한다.
   ```
   python scripts/fetch_stock_price.py --service-key <STOCK_PRICE_SERVICE_KEY> <stock_code> <YYYY1231>
   ```
   - `stock_code`는 `corp_code_lookup.py` 결과의 6자리 종목코드.
   - 공공데이터포털("금융위원회_주식시세정보") API를 쓴다. 데이터가 없는 날(휴장일·미갱신)이면 스크립트가 자동으로 며칠 전까지 거슬러 올라간다.
   - 이 API는 실시간이 아니라 기준일 다음 영업일 오후 1시 이후 갱신되지만, 이 스킬은 과거 연말 종가만 필요하므로 문제없다.
   - 이 호출은 `apis.data.go.kr`로 나간다. 네트워크 오류가 나면 Cowork 설정에서 이 도메인이 허용되어 있는지 사용자에게 먼저 확인한다.
   - 잘 안되면 `--raw` 옵션으로 원본 응답을 확인해 원인(키 미승인, 파라미터 오류 등)을 파악한다.
