...
- **"비교" 표현이 있음** (예: "삼성전자, LG화학, 카카오 비교해줘"): 1~4단계(기업 고유번호 조회 + 원자료 수집)는 기업마다 각각 수행해 `cache/`에 데이터를 쌓아두되, 5단계(엑셀 작성)에서는 `scripts/build_workbook.py`가 아니라 **`scripts/build_comparison_workbook.py`를 한 번만** 실행해 비교용 엑셀 파일 1개를 만든다.
  ...
  - **산출물은 눈에 보이는 시트 딱 2개**: `12분기 비교`, `최근5년 비교`. 각 시트 상단에 지표별·회사별 데이터 표(회사가 컬럼 블록, 지표가 행)가 있고, 그 아래에 지표 하나당 꺾은선 차트 하나(회사=계열)가 이어진다. 표시되는 지표는 매출액·매출원가·매출총이익·영업이익·당기순이익·유동자산·매출채권및기타채권·유동부채·경상이익·자산·부채·이익잉여금·현금및현금성자산의증가·자기자본비율·부채비율·매출총이익률·원가율·영업이익률·순이익률 총 19개이며, 비율 6개를 포함해 **전부 꺾은선 그래프**로 통일한다(막대그래프 사용 안 함). 회사별 원자료·상세 계산 시트는 감사(audit) 목적으로 존재하지만 전부 숨김 처리되어 있다.
