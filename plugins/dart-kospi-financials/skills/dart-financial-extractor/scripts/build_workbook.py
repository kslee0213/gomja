        ...
        for acc in accounts:
            ...
            for i, p in enumerate(quarter_plan):
                ...
                if sj_div == "BS":
                    ...
                    # ⚠ 회사에 따라 반기보고서(11012)의 thstrm_amount가 "2분기 3개월 단독"이고
                    # 누적(6개월)은 thstrm_add_amount에 있는 경우가 있다(HD한국조선해양에서 실제 확인
                    # — 기존 가정(thstrm=반기누적)과 반대). 누적 필드가 있으면 그것을 누적으로 쓴다:
                    # 2분기 = 누적(add 있으면 add, 없으면 thstrm) − 1분기.
                    h1_cum_ref = _cell_ref_with_alias(cell_index, sj_div, acc, f"{label}h1", "thstrm_add_amount")                         or _cell_ref_with_alias(cell_index, sj_div, acc, f"{label}h1", "thstrm_amount")
                    q1_ref = _cell_ref_with_alias(cell_index, sj_div, acc, f"{label}q1", "thstrm_amount")
                    if h1_cum_ref and q1_ref:
                        cell.value = f"={h1_cum_ref}-{q1_ref}"
                        cell.font = BLACK
                elif q == 4:
                    ...
                    if fy_ref and q3_add_ref:
                        ...
                        cell.font = BLACK
                    elif fy_ref and i >= 3 and [
                        (quarter_plan[i - 3 + k]["year"], quarter_plan[i - 3 + k]["q"]) for k in range(3)
                    ] == [(p["year"], 1), (p["year"], 2), (p["year"], 3)]:
                        # 현금흐름표 항목처럼 3분기보고서에 누적 필드가 없는 경우가 있다
                        # (분기보고서 자체가 이미 연초 누적치로 공시되는 계정 등).
                        # 이 경우 같은 행에 이미 계산해 둔 1~3분기 값을 빼는 방식으로 대체한다:
                        # 4분기 = 연간 - 1분기 - 2분기 - 3분기.
                        # ⚠ 같은 연도의 1~3분기 열이 실제로 시트에 있을 때만 쓴다 — 12분기 창의 첫 연도처럼
                        # 앞 분기가 잘려 있으면 라벨/빈 열을 빼서 #VALUE!가 나므로(HD한국조선해양에서 확인) 빈 칸으로 둔다.
...
def main() -> None:
    ...
    if want_annual and year_list:
        ...
        if latest_code and latest_code != "11011":
            ...
            # 3분기보고서는 원래 누적 필드를 쓰고, 1분기/반기보고서도 누적 필드(thstrm_add_amount)가
            # 실제로 공시돼 있으면 그것을 쓴다(회사에 따라 thstrm_amount가 3개월 단독인 경우가 있음).
            def _has_add(data):
                return any("thstrm_add_amount" in it for sj in ("IS", "CIS", "CF") for it in data.get("items", {}).get(sj, []))
            field = "thstrm_add_amount" if (latest_code == "11014" or _has_add(cur_data)) else "thstrm_amount"
